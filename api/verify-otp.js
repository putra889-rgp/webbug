// api/verify-otp.js
const { otpStore, verifiedStore } = require("./_shared_otp_store");
const MAX_VERIFY_ATTEMPTS = parseInt(process.env.MAX_VERIFY_ATTEMPTS || "3", 10);
const VERIFIED_TTL_SECONDS = parseInt(process.env.VERIFIED_TTL_SECONDS || "300", 10); // how long verification valid
...
module.exports = async (req, res) => {
  ...
  // success: consume OTP and mark verified
  otpStore.delete(number);
  const verifiedUntil = Date.now() + VERIFIED_TTL_SECONDS * 1000;
  verifiedStore.set(number, { verifiedUntil });

  return res.status(200).json({ ok: true, message: "OTP valid, number verified" });
};
